...
function sumOfCodes(str) {
  ...
  return typeof str === 'string'
    ? str.split('').reduce((acc, item) => acc + item.charCodeAt(0), 0)
    : 0;
}
...
function countVowels(str) {
  // throw new Error('Not implemented');
  return str.split('').filter((item) => 'aeiouy'.includes(item.toLowerCase()))
    .length;
}
...
function isPalindrome(str) {
  ...
  return (
    str
      .toLowerCase()
      .split('')
      .filter((item) => 'abcdefghijklmnopqrstuvwxyz'.includes(item))
      .join('') ===
    str
      .toLowerCase()
      .split('')
...
function encodeToRot13(str) {
  // throw new Error('Not implemented');
  const aplhaUp = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  const aplhaDown = 'abcdefghijklmnopqrstuvwxyz';
  const arrayStr = str.split('');
  let ans = '';
  for (let index = 0; index < arrayStr.length; index += 1) {
    if (aplhaUp.indexOf(arrayStr[index]) !== -1) {
      ans += aplhaUp[(aplhaUp.indexOf(arrayStr[index]) + 13) % 26];
    } else if (aplhaDown.indexOf(arrayStr[index]) !== -1) {
      ans += aplhaDown[(aplhaDown.indexOf(arrayStr[index]) + 13) % 26];
    } else {
      ans += arrayStr[index];
    }
  }
...
function getCardId(value) {
  ...
  if (value.length !== 3) {
    return (!dir[value[0]] ? +value[0] : +dir[value[0]]) + dir[value[1]] - 1;
  }
  return 10 + dir[value[2]] - 1;
}
